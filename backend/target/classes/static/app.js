...
const API = '';           // mismo origen (Spring Boot sirve el HTML)
...
function dibujarMapa(posiciones) {
  ...
  ctx.lineWidth = 1;
  for (let i = 0; i <= 10; i++) {
    const x = (W / 10) * i, y = (H / 6) * i;
    ctx.beginPath(); ctx.moveTo(x, 0); ctx.lineTo(x, H); ctx.stroke();
    if (i <= 6) { ctx.beginPath(); ctx.moveTo(0, y); ctx.lineTo(W, y); ctx.stroke(); }
  }

  // Título del mapa
  ctx.fillStyle = 'rgba(139,148,158,.5)';
  ctx.font = '12px Inter';
  ctx.fillText('Área metropolitana de Guayaquil', 12, 20);
  ...
  ctx.lineWidth = 1;
  ctx.setLineDash([4, 4]);
  ctx.beginPath(); ctx.arc(baseX, baseY, 20, 0, Math.PI * 2); ctx.stroke();
  ctx.setLineDash([]);
  ctx.fillStyle = 'rgba(255,255,255,.4)';
  ctx.font = '10px Inter';
  ctx.fillText('Guayaquil', baseX + 12, baseY + 4);
  ...
  if (!posiciones || posiciones.length === 0) {
    ...
    ctx.fillText('Sin datos GPS — el simulador publicará pronto...', W / 2, H / 2);
    ctx.textAlign = 'left';
    return;
  }

  posiciones.forEach(p => {
    const color = COLORS[p.vehiculoId] || '#888';
    ...
    ctx.fillText(p.vehiculoId, x + 11, y + 4);
    ctx.fillStyle = 'rgba(230,237,243,.5)';
    ctx.font = '9px JetBrains Mono';
    ctx.fillText(`${(p.velocidadKmh||0).toFixed(0)} km/h`, x + 11, y + 15);

    // Leyenda
    const li = document.createElement('div');
    li.className = 'leyenda-item';
    li.innerHTML = `<div class="leyenda-dot" style="background:${color}"></div>
      <span><b>${p.vehiculoId}</b> — (${p.latitud.toFixed(4)}, ${p.longitud.toFixed(4)})</span>`;
...
async function fetchAll() {
  await fetchStatus();
  await fetchAlertas();
  // Vehiculos solo si esa vista está activa (para no sobrecargar)
...
async function healthCheck() {
  try {
    ...
    if (d.status === 'UP') {
      addLog('✅ API Spring Boot conectada — sistema online', 'log-gps');
      addLog('📡 Simulador publicando en MQTT cada 5s', 'log-bridge');
      addLog('🔀 Bridge reenviando alertas → RabbitMQ', 'log-bridge');
      setStatus(true);
    }
  } catch(e) {
    addLog('⚠️ API no disponible — asegúrate de ejecutar ./start.sh', 'log-alert');
    setStatus(false);
  }
}
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  // Auto-refresh cada 5s (sincronizado con el simulador)
  setInterval(async () => {
    await fetchAll();
