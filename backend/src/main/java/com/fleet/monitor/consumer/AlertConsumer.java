 ...
 *
 * cola.temperatura (temp.alert) → detecta si alerta=true → reenvía a
 * cola.notificaciones
 * cola.combustible (fuel.routing)→ detecta si alerta_bajo=true → reenvía a
 * cola.notificaciones
 *
...
public class AlertConsumer {
    ...
    public void consumirTemperatura(Map<String, Object> datos) {
        try {
            ...
            double temperatura = toDouble(datos.get("temperaturaC"));
    ...
    public void consumirCombustible(Map<String, Object> datos) {
        try {
            String vehiculoId = (String) datos.getOrDefault("vehiculoId", "DESCONOCIDO");
            double nivel = toDouble(datos.get("nivelPct"));
            boolean esAlerta = Boolean.TRUE.equals(datos.get("alerta"));
